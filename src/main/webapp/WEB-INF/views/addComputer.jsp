		...
		<div class="container">
			...
			<div class="row">
				<div class="col-xs-8 col-xs-offset-2 box">
					<h1>${addComputer}</h1>
					<form action="${contextPath}/computer/add"
						method="POST">
						<fieldset>
							<div
								class="form-group ${fn:contains(errors, 'name') ? 'has-error': ''}">
								<label class="control-label" for="computerName">${columnName}</label> <input type="text" class="form-control"
									name="name" id="name"
									placeholder="${columnName}" value="${computer.name}">
							</div>
							<div
								class="form-group ${fn:contains(errors, 'introduced') ? 'has-error': ''}">
								<label class="control-label" for="introduced">${columnIntroduced}</label> <input type="date" class="form-control" name="introduced"
									id="introduced" placeholder="${columnIntroduced}"
									value="${computer.introduced}">
							</div>
							<div
								class="form-group ${fn:contains(errors, 'discontinued') ? 'has-error': ''}">
								<label class="control-label" for="discontinued">${columnDiscontinued}</label> <input type="date" class="form-control"
							...
							<div
								...
								<label class="control-label" for="companyId">${columnCompany}</label> <select
									...
									<c:forEach items="${companies}" var="company">
										<option value="${company.idaa}">${company.name}</option>
									</c:forEach>
								</select>
							</div>
						</fieldset>
